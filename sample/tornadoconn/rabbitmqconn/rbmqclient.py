...
class RbmqClient:
    ...
    def _connect(self):
        try:
            conn = TornadoConnection(
                pika.URLParameters(self._url), on_open_callback=self.on_connection_open
            )
    ...
    def control_queue(
        self, queue_name: str, callback: Callable[[Channel], None] = None
    ) -> None:
    ...
    def create_queue_consumer(
        self, queue_name: str, workerfunc: Callable[[list[Mapping[str, Any]]], None]
    ) -> None:
